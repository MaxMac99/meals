...
|---:|---|---|---|---|
| — | — | — | Unreleased | **The built-in assistant, in the tree and not yet built.** A sixth tab ("Assistant") holding a chat over the server's new `POST /assistant/chat`: a server-side agent that runs the same MCP tools the `/mcp` endpoint publishes, in-process, against the caller's own household. The chat sends the recent conversation, renders the answer with read-only reference chips for the meals/recipes/plan it touched, and shows a native confirmation dialog when the server holds a destructive action (delete recipe/meal/ingredient, merge, archive the list) — the confirmed envelope goes back verbatim and the server re-validates it. A server without an LLM provider configured answers 404 and `/client-config` says `assistant_enabled: false`; the tab shows its disabled screen and everything else works. **No build has been uploaded with this yet** — it rides the next one; bump `CFBundleVersion` per the ritual above when it ships. |
